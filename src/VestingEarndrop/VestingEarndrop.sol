...
contract VestingEarndrop is Ownable2Step, EIP712, ReentrancyGuard {
    ...
    constructor(
        ...
        address _signer,
        address _treasurer
    ) Ownable(_owner) EIP712("Galxe Vesting Earndrop", "1.0.0") {
     ...
     */
    function setEarndropRevocable(
        uint256 earndropId,
        bool revocable
    ) external onlyOwner earndropExists(earndropId) notRevoked(earndropId) {
    ...
    function activateEarndrop(
        ...
        bool isVerified = _verifySignature(
            _hashEarndropActivate(earndropId, tokenAddress, merkleTreeRoot, totalAmount, _stagesArray, admin),
            _signature
        );
        if (!isVerified) {
     ...
     * @param earndropId The unique ID of the Earndrop.
     */
    function confirmActivateEarndrop(
        uint256 earndropId
    ) external payable earndropExists(earndropId) notRevoked(earndropId) onlyAdmin(earndropId) {
        Earndrop storage earndrop = earndrops[earndropId];
     ...
     */
    function transferEarndropAdmin(
        uint256 earndropId,
        address newAdmin
    ) external earndropExists(earndropId) onlyAdmin(earndropId) {
        Earndrop storage earndrop = earndrops[earndropId];
     ...
     */
    function revokeEarndrop(
        uint256 earndropId,
        address recipient
    ) external earndropExists(earndropId) notRevoked(earndropId) onlyAdmin(earndropId) isConfirmed(earndropId) {
        Earndrop storage earndrop = earndrops[earndropId];
    ...
    function claimEarndrop(
        ...
        bytes32 leaf = keccak256(
            abi.encodePacked(earndropId, params.stageIndex, params.leafIndex, params.account, params.amount)
        );
    ...
    function multiClaimEarndrop(
        ...
        for (uint256 i = 0; i < params.length; i++) {
            ...
            bytes32 leaf = keccak256(
                abi.encodePacked(earndropId, claim.stageIndex, claim.leafIndex, claim.account, claim.amount)
            );
    ...
    function _hashEarndropClaim(uint256 earndropId, uint256 leafIndex, uint256 value) private view returns (bytes32) {
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
                        keccak256("EarndropClaim(uint256 earndropId,uint256 leafIndex,uint256 value)"),
                        earndropId,
                        leafIndex,
                        value
                    )
